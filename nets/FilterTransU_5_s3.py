...
class FilterTransU_5_s3(nn.Module):
    ...
    def __init__(self, in_chan, base_chan, num_classes=1, reduce_size=7, block_list='4', num_blocks=[1, 2, 4],
        ...
        self.down4 = down_block_4(8 * base_chan, 16 * base_chan, (2, 2), num_block=2)
    ...
    def forward(self, x):
        ...
        # # outc = self.sig(outc)
        # # -------------Classification-------------
        # cls_branch = self.cls(out).squeeze(3).squeeze(2)  # (B,N,1,1)->(B,N)
        # cls_branch_max = cls_branch.argmax(dim=1)
        # cls_branch_max = cls_branch_max[:, np.newaxis].float()
        # out = self.dotProduct(outc,cls_branch_max)
        # out = self.sig(out)
        # return out
